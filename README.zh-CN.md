...
| **点一下** | 打开会话清单——有哪些 DSH 会话、哪个活着、在干什么。再点一下收起。 |
| **右键** | 循环换皮肤。 |
...
宠物盯着 `~/.dsh`——进程、会话活动、一个可选的提示文件——把看到的映射成六个状态。想手动驱动：
...
只有桌面这只在观察 DSH。它把看到的写进 `~/.dsh-desk-pet/state.json`，网页那只读这个文件——「agent 在干什么」只有一份实现，而不是两份会互相打架的。
...
```bash
/usr/bin/python3 -m unittest discover -s tests -v          # 144 个测试，不需要显示器
...
**generate_frames** 从不凭空重画角色：每次请求都是拿一张已有的图做 image-to-image，因为文生图跨次调用锁不住身份。状态的第一帧参考本套皮肤的 idle 姿势，第二帧参考**它自己的第一帧**——循环要的是同一个姿势差一瞬间，不是两个不同姿势。
